"""
Tests for atan2_cordic_16b.

The 8-bit atan2_cordic tests were removed along with the module itself:
nothing in main.v instantiates it (echo_angle_detector uses the 16-bit
version directly), so it is no longer in src/ or in PROJECT_SOURCES.
"""
...
async def atan2_16_cordic_test(dut, x: int = 1, y: int = 1):
    ...
    assert formatted_result is not None, (
        'No angle returned within 8 cycles by atan2 16'
    )
